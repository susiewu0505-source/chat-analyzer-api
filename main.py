        ...
        with open(filepath, encoding="utf-8") as f:
                    for line in f:
                                    ...
                                    for action in replay.get("actions", []):
                                                        ...
                                                        if r:
                                                                                text = "".join(run.get("text","") or (run.get("emoji",{}).get("shortcuts",[""])[0] if run.get("emoji",{}).get("shortcuts") else "") for run in r.get("message",{}).get("runs",[])).strip()
                                                                                if text: msgs.append({"text":text,"author":r.get("authorName",{}).get("simpleText",""),"timeMs":ms,"type":"chat","amount":0})
...
def download_chat(req: AnalyzeRequest):
    ...
    if not vid: raise HTTPException(status_code=400, detail="无法解析YouTube URL")
        ...
        if not files: raise HTTPException(status_code=404, detail="未找到弹幕数据")
                    msgs = parse_ndjson(str(files[0]))
        if not msgs: raise HTTPException(status_code=404, detail="弹幕为空")
